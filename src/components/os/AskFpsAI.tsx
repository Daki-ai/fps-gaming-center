...
const AskFpsAI = () => {
  const quickQuestions = [
    "What is the best PC for CS2?",
    "How much is Elite Zone?",
    "Show me pro configs",
    "When is the next tournament?",
    "How should I warm up before FACEIT?",
    "What are the rules?",
  ];
  ...
  return (
    <section id="ask-ai" className="px-4 py-20 md:py-28">
      <div className="max-w-6xl mx-auto">
          ...
          <div className="relative bg-black/60 border border-gold/30 rounded-2xl p-8 backdrop-blur-xl">
            ...
            <div className="bg-black/50 border border-white/5 rounded-xl p-6 mb-6 min-h-[200px] flex flex-col items-center justify-center text-center">
              ...
              <p className="text-sm text-gray-500 max-w-md">
                In the meantime, ask our staff or check the sections below for everything you need to know about your session.
              </p>
            </div>
        ...
        <div>
          <p className="text-center text-sm text-gray-500 mb-6 uppercase tracking-wider">Quick Questions</p>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {quickQuestions.map((q, i) => (
              <button
                key={i}
                className="group text-left px-5 py-4 bg-black/40 border border-white/5 hover:border-gold/40 rounded-xl backdrop-blur-md transition-all duration-300 hover:bg-black/60"
              >
